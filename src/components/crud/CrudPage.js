...
function CrudPage({ config }) {
  ...
  const downloadTemplate = async () => {
    try {
      ...
      showFeedback({
        type: "error",
        title: "Error",
        message: getSafeApiErrorMessage(error, "Unable to download template."),
        color: "danger",
        action: null,
      });
    }
  ...
  const handleChange = (e) => {
    if (e.target.type === "checkbox") {
      ...
      setForm((currentForm) => {
        const currentValues = currentForm[name] || [];
        ...
        return { ...currentForm, [name]: nextValues };
      });
    } else {
    ...
    try {
      ...
      showFeedback({
        ...
        message: getSafeApiErrorMessage(
          error,
          `Failed to load ${config.entityName.toLowerCase()}.`,
        ),
        color: "danger",
        action: null,
      });
  ...
  useEffect(() => {
    if (!canView || isForbidden || (!canCreate && !canUpdate)) {
      return;
    }

    const optionFields = config.fields.filter(
      (field) =>
        ["select", "multi-select", "checkbox-group"].includes(field.type) &&
      ...
      apiClient
        ...
        .catch((error) => {
          showFeedback({
            ...
            message: getSafeApiErrorMessage(
              error,
              `Failed to load ${field.label.toLowerCase()} options.`,
            ),
            color: "danger",
            action: null,
          });
  ...
  }, [config.fields, canCreate, canUpdate, canView, isForbidden]);
  ...
  useEffect(() => {
    ...
    dataTable.current = $(tableElement).DataTable({
      ...
      layout: {
        ...
        bottomEnd: {
          className:
            "dt-layout-end col-md-auto ms-auto d-flex justify-content-end align-items-center gap-3",
    ...
    $(tableElement).on("click", ".crud-expand", function () {
      ...
      if (row.child.isShown()) {
        ...
        row
          .child(
            expandablePanel(row.data().employees, currentConfig.expandableRows),
          )
          .show();
            ...
            <div className="datatable-toolbar">
              ...
              {config.entityName === "Attendance" && (
                <>
                  ...
                  <button
                    className="btn btn-warning btn-sm btn-with-icon"
              ...
              {canCreate && (
                <button
                  className="btn btn-success btn-sm px-3 btn-with-icon"
